class SelectionRestorerFromText {
    ...
    public restoreRange (selection: Selection, onlyReturnRange?: boolean) {
        ...
        if(this.parentNodeStart === undefined || this.parentNodeEnd === undefined ) return
        this.getRange(this.parentNodeStart, 0, true)
        if(this.endContainer !== this.parentNodeStart){
            this.getRange(this.parentNodeEnd, 0, false)
        }
        else{
            this.range.setEnd(this.parentNodeEnd, this.parentNodeEnd.childNodes.length) 
        }
    ...
    private getRange (node: Node, offset: number, startOffset: boolean){
        for (let i = 0; i < node.childNodes.length; i++) {
            ...
            if (child.nodeType === Node.TEXT_NODE) {
                if(startOffset){
                    if (offset + child.textContent!.length >= this.startOffset && offset <= this.startOffset) {
                        this.range.setStart(child, this.startOffset - offset )
                    }
                }
                else{
                    if (offset + child.textContent!.length >= this.endOffset && offset <= this.endOffset) {
    ...
    private getOffset(node: Node, container: Node, offset: number, startOffset: boolean) {
...
const getParentNodeWithTag = (node: Node, tags: string[]): Node | null => {
    ...
    while (currentNode && !tags.some((value) => value.toUpperCase() === currentNode.nodeName)) {
...
const deleteAnyChildNodesOfTag = (element: Node, range: Range, tag: string): boolean => {
...
const splitNodeBetweenRange = (node: Node, range: Range) => {
    ...
    const nodeIsPartlyInRange = nodeRange.compareBoundaryPoints(Range.START_TO_END, range) > 0 
    && nodeRange.compareBoundaryPoints(Range.START_TO_START, range) === -1
    
    if (nodeIsPartlyInRange) { // before
...
const createRangeFromSelectionEndToNodeEnd = (node: Node, selection: Selection) => {
...
const checkIfTextInsideOfTag = (range: Range, tag: string) => {
    ...
    if(parentNode?.textContent === range.toString() && parentNode?.nodeName === tag.toUpperCase()){
    ...
    while (parentNode) {
        if (parentNode.nodeName === tag.toUpperCase()) {
            if(parentNode?.textContent !== range.toString()){
                ...
                const rangeFromEndOfSelectionToEndOfNode = createRangeFromSelectionEndToNodeEnd(parentNode, selection);
    ...
    const wasTagDeletedHoveringWholeSelection = deleteAnyChildNodesOfTag(element, range, tag)
...
const surroundNodeInRangeWithTag = (mainNode: Node, tag: string, range: Range) => {
...
const splitNodesInRange = (node: Node, range: Range) => {
    ...
    for(let i=0; i<childrenOfParent.length; i++){
        ...
        const elementIsBeforeRange = nodeRange.compareBoundaryPoints(Range.START_TO_END, range) < 0
        ...
        if(nodeIsPartlyInRange){
            ...
            const deepestChildOfAfterSplit = getDeepestNonTextNode(copyOfChild) as HTMLElement
  
            if(deepestChildOfAfterSplit.TEXT_NODE){ // it means that node is text
                deepestChildOfAfterSplit.textContent  = getRangeInnerHTML(rangeAfterSplit)
            }else{
            ...
            if(deepestChild.TEXT_NODE){ // it means that node is text
...
const groupElementsByParent = (elements: HTMLElement[]): Map<HTMLElement | null, HTMLElement[]> => {
...
const createRangeFromNodeAndRange = ({range, node, fromStartOfRange}:{range: Range, node: Node, fromStartOfRange: boolean}): Range => {
    const nodeRange = document.createRange();
    nodeRange.selectNodeContents(node);
...
const getFurthestParentNodeContainingOnlyText = (node: Node, text: string) => {
    ...
    while (currentNode.parentNode && currentNode.parentNode.textContent === text) {
...
const getNodesInRange = (range: Range, tagName: string[], furthestParentNode: Node) => {
    ...
    for (node ; node; node = node.parentNode as Node)
    {
        ...
        if (node.parentNode == furthestParentNode)
            break;
    }
    nodes.reverse();
    ...
    {
        if(tagName.some((value) => value === node.nodeName) && range.intersectsNode(node))
        {
            nodes.push(node);
        }  
    }
...
const changeTag = (element: Node, newTag: string) => {
    ...
    for (let i = 0; i < convertedElement.attributes.length; i++) {
        newElement.setAttribute(convertedElement.attributes[i].name, convertedElement.attributes[i].value);
    }
...
const isNodeInsideRange = (range: Range, node: Node): boolean => {
    ...
    const compareStart = range.compareBoundaryPoints(Range.START_TO_START, nodeRange);
    const compareEnd = range.compareBoundaryPoints(Range.END_TO_END, nodeRange);
    return compareStart <= 0 && compareEnd >= 0;
}

const makeElementHaveSameStyleAsOther = (elementRestyled: Node, element: Node) => {
    ...
    for (let i = 0; i < convertedElement1.attributes.length; i++) {
        convertedElement2.setAttribute(convertedElement1.attributes[i].name, convertedElement1.attributes[i].value);
    }
}


const styleSpans = ({range, styleProperty, styleValue, allSpansInsideSelection,  sameValues, allowCollapsedRange=false, callbackFunction,}:
    {
        range: Range, 
        ...
        callbackFunction?: (element: HTMLElement, styleProperty: string, styleValue: string, sameValues: boolean) => void|undefined
    }) => {
    
    allSpansInsideSelection.forEach((element)=>{
        let elementConverted = element as HTMLElement
      
        if(isNodeInsideRange(range, element) || element.textContent === range.toString()){
            if(callbackFunction){
                callbackFunction(elementConverted as HTMLElement, styleProperty, styleValue, sameValues)
            }else{
            ...
            rangeInElementTillStartORange.setEnd(miniRangeInside.startContainer, miniRangeInside.startOffset)
            ...
            rangeFromStartOfRangeToEndOfElement.setStart(miniRangeInside.endContainer,miniRangeInside.endOffset)
            rangeFromStartOfRangeToEndOfElement.setEnd(element, element.childNodes.length)
            
            const newContentOfSpan = getRangeInnerHTML(rangeFromStartOfRangeToEndOfElement)
            if(newContentOfSpan!==""){
                const span = elementConverted.cloneNode(true) as HTMLElement
            ...
            if(allowCollapsedRange && miniRangeInside.toString() === ""){
                const deepestChild = getDeepestNonTextNode(elementConverted) as HTMLElement
...
const checkIfAllElementsHaveSameValue = (range: Range, styleProperty: string, styleValue: string, elements: Node[], allowCollapsedRange = false) => {
...
const getCommonValueForElements = (range: Range, styleProperty: string, elements: Node[], allowCollapsedRange = false) => {
    let styleValue = null
    for(let i = 0; i < elements.length; i++){
        const miniRangeInside = getInnerRange(elements[i], range)
        if(miniRangeInside.toString()==="" && !allowCollapsedRange) continue
...
const hasAncestorSpan = (node: Node): boolean => {
    ...
    while (parentNode) {
      ...
      if(parentNode.nodeName === "LI" || parentNode.nodeName === "P") return false
      parentNode = parentNode.parentNode;
    }
    return false;
}
...
const getDeepestNonTextNode = (node: Node) => {
  ...
  function isBrBetweenTextNodes(textNode1: Node, textNode2: Node, rootNode: HTMLElement): boolean {
    let brFound = false;
...
const getTextOutsideRange = (textNode: Node, range: Range): { text: string, startingPosition: number } => {
    ...
    return {text: textBeforeRange + textAfterRange, startingPosition: textBeforeRange.length};
}
...
const findColumnParent = (element: HTMLElement) => {
    ...
    while (currentNode) {
        if (currentNode.nodeName!=="#text" && currentNode.classList.contains("column")) {
            return currentNode
        }
...
export {SelectionRestorerFromText, getParentNodeWithTag, deleteAnyChildNodesOfTag, deleteNode, getInnerRange,  isNodeInsideRange,
    splitNodeBetweenRange, checkIfTextInsideOfTag, addTagAroundRange, surroundNodeInRangeWithTag, createElementInRange, makeElementHaveSameStyleAsOther,
    splitNodes, createRangeFromNodeAndRange, getFurthestParentNodeContainingOnlyText, styleSpans, createRangeFromNode, findColumnParent, 
    getCommonValueForElements, hasTextBetweenNodes, createLinkFromString, getFurthestDeletableNode, getTextOutsideRange, isNestedFirstChild,
    getNodesInRange, changeTag, getRangeBetweenElements, checkIfAllElementsHaveSameValue, hasAncestorSpan, splitNodesInRange, 
    isBrBetweenTextNodes, getFirstSpanOfSelectionStart, groupElementsByParent}