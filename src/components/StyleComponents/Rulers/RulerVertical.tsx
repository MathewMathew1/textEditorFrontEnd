...
const RulerVertical = () => {
    ...
    const handleMouseDown = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
      ...
      const handleMouseMove = (e: MouseEvent) => {
        ...
        let newMargin = clamp((e.clientY - rect.top)/convertedScale, 0, ruler.current!.getBoundingClientRect().height - marginEnd - MARGIN_MINIMAL_DISTANCE)
    ...
    const handleMouseDownEndMargin = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        e.preventDefault();
    ...
    const handleMouseUpEndMargin = (e: MouseEvent) => {
        ...
        let newMargin = clamp((rect.bottom - e.clientY)/convertedScale, 0, ruler.current!.getBoundingClientRect().height - marginStart - MARGIN_MINIMAL_DISTANCE)
    ...
    return (
        <div style={{width: ruler.current?.style.width, top: `${(editorValues.currentPage-1)*297}mm`}} className='ruler-container-vertical' >   
            <div  ref={ruler}  className="ruler-vertical">
                <div onMouseDown={handleMouseDown} ref={elementRef} className='margin-ruler' style={{top: "0", height: Math.max(marginStart*convertedScale, 1)}}></div>
                <div onMouseDown={handleMouseDownEndMargin} ref={elementRef2} className='margin-ruler' style={{bottom: "0", height: Math.max(marginEnd*convertedScale, 1)}}></div>
                {ticks.map(tick =>
                (
                    <div key={`cm ${tick}`} style={{height: `${1 * convertedScale}cm`}} className="cm">
                        <span className="label">{tick}</span>
                        <div  style={{marginTop:  `${3 * convertedScale}mm`}} className="mm" />
                        <div  style={{marginTop:  `${3 * convertedScale}mm`}} className="mm" />
                    </div>
                ))}
