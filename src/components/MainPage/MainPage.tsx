...
const MainPage = () => {
  ...
  const {
    ref: renameModalRef,
    isComponentVisible: isRenameModalVisible,
    setIsComponentVisible: setIsRenameModalVisible,
  } = useComponentVisible<HTMLDivElement>(false);
  const {
    ref: deleteModalRef,
    isComponentVisible: isDeleteModalVisible,
    setIsComponentVisible: setIsDeleteModalVisible,
  } = useComponentVisible<HTMLDivElement>(false);
  ...
  const openCreateTextDocumentModal = async (
    ...
    <div className="main-page">
      {user.fetchingUserDataFinished ? (
        <>
          ...
          <div
            className="container"
            style={{ flexDirection: "column", backgroundColor: "#f1f3f4" }}
          >
            <div className="templates-container">
              ...
              <div className="docs-home-screen-content">
                {user.templates.map((template, index) => (
                  <div
                    className="template withoutInfo"
                    style={{}}
                    key={`template ${index}`}
                  >
              ...
              <div className="docs-home-screen-content">
                {user.userDocuments.length === 0 ? (
                  <div className="no-documents-found">
                    You have no documents created so far.
                  </div>
                ) : (
                  <>
                      ...
                      >
                        <div className="container-document-info">
                          ...
                          <div className="flex-row">
                            <File color={true} height={20} width={20} />
                            {formatDate(document.lastUpdatedAt)}
                            <div className="dots-container">
                              <button
                                onClick={() =>
                                  setDocumentDropDown(document._id)
                                }
                                className="button-div dots"
                              >
                                <ThreeDots />
                              </button>
                            </div>
                            {documentDropdown === document._id ? (
                              <div className="option-dropdown">
                                ...
                                >
                                  <TextDocumentLetter /> <div>Rename</div>
                                </button>
                                <button
                        ...
                        <div
                          onClick={() =>
                            navigate(`/textDocument/${document._id}`)
                          }
                          style={{ scale: "0.3", transformOrigin: "top left" }}
                          className="template-editor"
                          dangerouslySetInnerHTML={{
                            __html: user.logged ? document.text : "",
                          }}
                        ></div>
                      </div>
                    ))}
          ...
          <RenameModal
            isModalOpen={isRenameModalVisible}
            setIsModalOpen={setIsRenameModalVisible}
            innerRef={renameModalRef}
            titleName={renameModalInfo.title}
            idOfDocument={renameModalInfo.id}
          />
          <DeleteModal
            isModalOpen={isDeleteModalVisible}
            setIsModalOpen={setIsDeleteModalVisible}
            innerRef={deleteModalRef}
            titleName={deleteModalInfo.title}
            idOfDocument={deleteModalInfo.id}
          />
        </>
      ) : (
        <LoadingCircle />
      ...
      >
        <a
            style={{fontSize: "11pt"}}
          href="https://www.flaticon.com/free-icons/extention"
          title="extention icons"
          target="_blank"
          rel="noopener noreferrer"
        >
          Extention icons created by paonkz - Flaticon
        </a>
      </footer>
    </div>
  );
