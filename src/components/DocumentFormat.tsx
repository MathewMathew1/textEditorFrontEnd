...
const DocumentFormat = () => {
  ...
  const [cellResizeInfo, setCellResizerInfo] = useState<cellResizeInfoType>({
    top: 0,
    left: 0,
    bottom: 0,
    right: 0,
  ...
  const [imageResizerInfo, setImageResizerInfo] = useState<ImageResizeInfo>({
    top: 0,
    left: 0,
    width: 0,
    rotateDegree: 0,
  ...
  const handleMarkdownChange = (e: React.FormEvent<HTMLDivElement>) => {
    editorValues.textDocument.saveValue(
      editorValues.markdownInput.current!.innerHTML,
      false,
      false
    );
  };
  ...
  useEffect(() => {
    const resizeObserver = new ResizeObserver(() => {
      if (!textEditorArea.current || !editorValues.markdownInput.current)
        return;

      editorUpdate.setOffsetForRuler(
        editorValues.markdownInput.current?.getBoundingClientRect().left
      );
  ...
  const enterPressed = (event: KeyboardEvent) => {
    ...
    if (parentNodeOfStart?.nodeName === "P" && !event.shiftKey) {
      ...
      const nextNodeIsEmptyText =
        brCreated.nextSibling?.nodeName === "#text" &&
    ...
    const rangeTillEndOFParagraph = createRangeFromNodeAndRange({
      range: range,
      node: parentNodeOfStart,
      fromStartOfRange: true,
    });
    ...
    if (splitNode === undefined) {
      ...
      let parentNode = getFurthestParentNodeContainingOnlyText(
        startElement,
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      if (
        !selection ||
        !editorValues.markdownInput.current?.contains(selection.anchorNode)
      )
        return;
      ...
      if (event.key === "Backspace") {
        ...
        if (selection && selection.isCollapsed) {
          const columnParent = findColumnParent(
            range.startContainer as HTMLElement
          );
          const selectionStartsAtBeginningOfColumn =
            elementIsAtStartOfColumn(
              columnParent,
              range.startContainer as HTMLElement
            ) && range.startOffset === 0;
          const deletedElementIsSpanInTable =
            range.startOffset === 1 &&
            range.startContainer.parentNode?.parentNode?.nodeName === "TD";
          const previousSibling = range.startContainer.parentNode
            ?.previousSibling as HTMLElement;

          const previousSiblingIsStartOfColumn = previousSibling
            ? elementIsAtStartOfColumn(columnParent, previousSibling) &&
              range.startOffset === 0
            : false;

          if (
            selectionStartsAtBeginningOfColumn ||
            deletedElementIsSpanInTable ||
            (previousSiblingIsStartOfColumn &&
              (previousSibling.textContent === "" ||
                previousSibling.innerHTML === String.fromCharCode(8203)))
          ) {
            event.preventDefault();
          }
        } else {
  ...
  const elementIsAtStartOfColumn = (
    columnParent: HTMLElement | null,
    element: HTMLElement
  ) => {
    return (
      columnParent?.firstChild === element ||
      isNestedFirstChild(element, columnParent!)
    );
  };
  ...
  const moveTableResizer = (cell: HTMLTableCellElement) => {
    ...
    if (previousCell) {
      maxLeft =
        (previousCell.getBoundingClientRect().left -
          textEditorArea.current!.offsetLeft +
          20) /
        convertedScale;
    } else {
      maxLeft =
    ...
    if (newtRow) {
      maxBottom =
        (newtRow.getBoundingClientRect().bottom -
          textEditorArea.current!.getBoundingClientRect().top -
          16) /
        convertedScale;
    } else {
      maxBottom =
    ...
    const leftConverted =
      (left - textEditorArea.current!.offsetLeft) / convertedScale;
    const topConverted =
      (top - textEditorArea.current!.getBoundingClientRect().top) /
      convertedScale;
    const bottomConverted =
      (bottom - textEditorArea.current!.getBoundingClientRect().top) /
      convertedScale;
    const rightConverted =
      (right - textEditorArea.current!.offsetLeft) / convertedScale;

    setCellResizerInfo({
      initialHeight: cellHeight,
      initialWidth: cellWidth,
      maxLeft: maxLeft,
      maxRight,
      maxBottom,
      maxTop,
  ...
  const moveImageResizer = (image: HTMLImageElement) => {
    ...
    const leftConverted =
      (leftBeforeRotation - textEditorArea.current!.offsetLeft) /
      convertedScale;
    const topConverted =
      (topBeforeRotation -
        textEditorArea.current!.getBoundingClientRect().top) /
    ...
    const centerXConverted =
      (centerX - textEditorArea.current!.offsetLeft) / convertedScale;
    const centerYConverted =
      (centerY - textEditorArea.current!.getBoundingClientRect().top) /
      convertedScale;
    ...
    setImageResizerInfo({
      ...
      right: leftConverted + width,
      bottom: topConverted + height,
      height: image.offsetHeight,
      rotateDegree: newRotate,
      centerX: centerXConverted,
      centerY: centerYConverted,
      image: image,
      maxWidth: 111,
      maxHeight: 444,
    });
  ...
  const memoizedHandleKeyDown = useCallback(handleKeyDown, [
    editorValues.textDocument,
  ]);
  const memoizedOnClick = useCallback(handleClick, [editorValues.textDocument]);
  const memoizedResize = useCallback(handleResize, [
    imageResizerInfo.image,
    editorValues.scale,
  ]);
  ...
  const updatePages = async () => {
    ...
    if (container) {
      const spans =
        editorValues.markdownInput.current!.getElementsByTagName("span");
      ...
      for (let i = 0; i < spans.length; i++) {
        if (spans[i].classList.contains("page")) {
          ...
          ).filter((span) => span.classList.contains("column"));
      ...
      await Promise.all(imagePromises).then(() => {
        // waiting till images loaded so high will correct for them
        pageSpans.forEach(({ page, columns }, index) => {
          ...
          columns.forEach((column, columnIndex) => {
            ...
            if (columnOverflow > 0) {
              ...
              if (pageSpans.length <= index + 1) {
                // creating new page and adding conentn to it
                ...
                // moving contentn to next page

                const nextPage = pageSpans[index + 1];
                const columnIndexInNextPage = Math.min(
                  columnIndex,
                  nextPage.columns.length - 1
                );
                const columnInNextPage =
                  nextPage.columns[columnIndexInNextPage];

                const isFirstInColumnContinuation =
                  areNodesEqual(
                    overflowingContent[0],
                    columnInNextPage.firstChild as HTMLElement
                  ) &&
                  areNodesEqual(
                    overflowingContent[0].firstChild as HTMLElement,
                    columnInNextPage.firstChild?.firstChild as HTMLElement
                  );

                if (
                  overflowingContent.length === 1 &&
                  isFirstInColumnContinuation
                ) {
                  const elementToAddText = columnInNextPage.firstChild
                    ?.firstChild as HTMLElement;

                  const elementAddingTextFrom = overflowingContent[0]
                    .firstChild as HTMLElement;
                  const textToAdd = elementAddingTextFrom.textContent || "";
                  elementToAddText.insertAdjacentText("afterbegin", textToAdd);
                } else {
                  overflowingContent.forEach((content) => {
                    columnInNextPage.insertBefore(
                      content,
                      columnInNextPage.firstChild
                    );
                  });
                }
              }
  ...
  const findOverflowingContent = (column: HTMLElement, page: HTMLElement) => {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      const totalBottomSpacing =
        parseFloat(getComputedStyle(childElement).paddingBottom) +
      ...
      for (let a = 0; a < childElement.children.length; a++) {
        ...
        const bottomOfThisChild =
          childOfChildElement.getBoundingClientRect().bottom +
          totalBottomSpacing;
        ...
        if (bottom - bottomOfThisChild < 0) {
          ...
          const childIsList =
            childElement.nodeName === "OL" || childElement.nodeName === "UL";
          if (a === 0 && childIsList) {
            removedElements.push(childElement);
          ...
          for (
            let indexRemainingChild = a;
            indexRemainingChild < remainingChildren.length;
            indexRemainingChild++
          ) {
            const child = remainingChildren[indexRemainingChild] as HTMLElement;
            const topOfThisChild = child.getBoundingClientRect().top;
            const isTextSpan =
              child.nodeName === "SPAN" && childElement.nodeName === "P";

            if (isTextSpan) {
              const overflownText = getOverflowingText(
                child,
                bottom - topOfThisChild - totalBottomSpacing
              );
      ...
      if (elementsToRemoveInside.length > 0) {
        ...
        if (toRemove.nodeName === "OL") {
          (toRemove as HTMLUListElement).setAttribute(
            "start",
            (childElement.childNodes.length + 1).toString()
          );
        }
  ...
  const getOverflowingText = (
    htmlElement: HTMLElement,
    remainingPossibleHeight: number
  ) => {
    ...
    while (startIndex > 1) {
      ...
      const isTextAboveBottomOfPage =
        remainingPossibleHeight >
  ...
  return (
    <div
      ref={documentArea}
      style={{ height: `calc(100vh - ${heightOfOffset}px)` }}
    ...
    >
      <div className="document-area">
        ...
        <div
          style={{
            width: `${
              editorWidth + COMMENT_HOLDER_WIDTH + PRE_EDITOR_AREA + 20
            }px`,
          }}
        ...
        >
          ...
          <div
            ref={paperAreaRef}
            style={{ width: PRE_EDITOR_AREA }}
            className="left-sidebar-container-content"
          ></div>
          <div
            onResize={(e) => changeOffsetRuler()}
            ref={textEditorArea}
            className="paper-area-container"
            style={{
              transform: `scale(${editorValues.scale})`,
              width: `calc(( 100vw - ${COMMENT_HOLDER_WIDTH}px - 100px - 40px ) / ${
                parseInt(editorValues.scale) / 100
              })`,
          ...
          >
            <div
              style={{ zIndex: 0, height: `${numberPages * 297}mm` }}
              className="backdrop"
            >
              <div
                ref={backdrop.highLightTextContainer}
            ...
            <TableResizer
              cellInfo={cellResizeInfo}
              show={showTableCellResizer}
            ></TableResizer>
            <ImageResizer
              show={showImageResizer}
              imageResizerInfo={imageResizerInfo}
            />
            {Array.from({ length: numberPages - 1 }, (_, index) => (
              <div
                key={`pageLine${index}`}
                style={{ top: `${297 * (index + 1)}mm` }}
                className="page-line"
              ></div>
            ))}
          </div>
          <div
            style={{
              width: `${COMMENT_HOLDER_WIDTH}px`,
              position: "absolute",
              left: editorWidth + PRE_EDITOR_AREA + 20,
              top: 0,
            }}
          >
            {" "}
